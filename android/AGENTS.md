...
- Legacy Desktop/CLI subagents may be hidden only with a matching direct parent `delegate_task` call: exact initial user request equals a task goal, creation within five seconds of that call, original first message near creation, same normalized profile, no fork/compression marker. Titles and plain parenthood are insufficient. Keep ambiguous rows visible. Proofs drive only the existing subagent list toggle; preserve raw source, capabilities and cached histories through additive Room v4 migration. Proof lookups are read-only, serialized and budgeted; downloaded transcripts are not retained by the classifier.
